...
def run():
    ...
    region_file = CUR_PATH + '/Input/Region.xlsx' # might need to add period back in before ./Input
    release_file = CUR_PATH + '/Input/ChemRelease.xlsx' # might need to add period back in before ./Input
    start_date_txt = real_start_date.get()
    end_date_txt = real_end_date.get()
    file_name = outfile_name.get()
    output_file_path = CUR_PATH + '/Output/' + file_name + '/' # might need to add period back in before ./Input
    start_date_txt = real_start_date.get()
...
def verify_input():
    ...
    start_day = datetime.strptime(start_date_temp, "%Y %m %d")
    end_day = datetime.strptime(end_date_temp, "%Y %m %d")
    sim_days = (end_day - start_day).days + 1

    if chem_type != 'Nanomaterial':
        load_data_nonNano = LoadData(chem_type, chem_file, region_file, release_file, start_date_temp, end_date_temp, sim_days)
...
def get_chemFile(chem_type):
    if chem_type == 'NonionizableOrganic':
        chem_file = CUR_PATH + '/Input/ChemParam_nonionizableOrganic.xlsx' # might need to add period back in before ./Input
    elif chem_type == 'IonizableOrganic':
        chem_file = CUR_PATH + '/Input/ChemParam_ionizableOrganic.xlsx' # might need to add period back in before ./Input
    elif chem_type == 'Metal':
        chem_file = CUR_PATH + '/Input/ChemParam_metal.xlsx' # might need to add period back in before ./Input
    else:
        chem_file = CUR_PATH + '/Input/ChemParam_nanomaterial.xlsx' # might need to add period back in before ./Input
    return chem_file
