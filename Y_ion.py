...
compart_list = ['air', 'rw', 'rwSed', 'fw', 'fwSed', 'sw', 'swSed', 'soil1', 'deepS1', 
			    'soil2', 'deepS2', 'soil3', 'deepS3', 'soil4', 'deepS4']
...
class Y_Value:
    ...
    ''' Species fraction '''
    def X_ij(self):
        solid_compart_list = ['rwSed', 'fwSed', 'swSed', 'soil1', 'soil2', 'soil3', 'soil4']
    ...
    def Kd_ij(self):
        ...
        if self.chem_type != 'Metal':
            ...
            Kd_n_compart = ['rw', 'rwSed', 'fw', 'fwSed', 'sw', 'swSed', 'soil1', 'deepS1', 'soil2', 'deepS2', 'soil3',
                            'deepS3', 'soil4', 'deepS4']
            ...
            # Kd [1] ionic
            Kd_i_compart = ['air', 'rw', 'rwSed', 'fw', 'fwSed', 'sw', 'swSed', 'soil1', 'deepS1', 'soil2', 'deepS2', 'soil3',
                            'deepS3', 'soil4', 'deepS4']
            Kd_i_water = ['air', 'rw', 'rwSed', 'fw', 'fwSed', 'sw', 'swSed']
    ...
    def Z_ij(self):
        ...
        if self.chem_type != 'Metal':
            Z_dict_sub = {'air': [Z_air, 0.0, 1.0], 'aer': [0.0, 0.0, 1.0], 'rw': [1.0, 1.0, 1.0], 'rSS': [0.0, 0.0, 1.0],
            			  'rSedW': [1.0, 1.0, 1.0], 'rSedS': [0.0, 0.0, 1.0], 'fw': [1.0, 1.0, 1.0], 'fSS': [0.0, 0.0, 1.0],
            ...
            Z_dict_sub['rSS'][0:2] = Kd_dict['rw']
            Z_dict_sub['rSedS'][0:2] = Kd_dict['rwSed']
            ...
            for i in range(0, 2):
                ...
                Z_dict['rw'][i] = self.env_dict['rwVf'] * Z_dict_sub['rw'][i] + \
                				  self.env_dict['rSSVf'] * Z_dict_sub['rSS'][i]
                Z_dict['rwSed'][i] = (1 - self.env_dict['riversedpercSolid']) * Z_dict_sub['rSedW'][i] + \
                                     self.env_dict['riversedpercSolid'] * Z_dict_sub['rSedS'][i]
    ...
    def Y_ij(self):
        ...
        for compart in compart_list:
            if self.chem_type != 'Metal':
                ...
                # for rw compartment, get the average value of Z_dict["rw"][0]
                if compart == "rw":
                    for i in range(len(Z_dict[compart][0])):
                        if Z_dict[compart][0][i] == 0:
                            Z_dict[compart][0][i] = 10 ** (-20)
                        if Z_dict[compart][1][i] == 0:
                            Z_dict[compart][1][i] = 10 ** (-20)
                    sum_val = X_dict[compart][0] / np.average(Z_dict[compart][0]) + X_dict[compart][1] / np.average(Z_dict[compart][1])
                    Y_dict[compart][0] = (X_dict[compart][0] / np.average(Z_dict[compart][0])) / sum_val
                    Y_dict[compart][1] = (X_dict[compart][1] / np.average(Z_dict[compart][1])) / sum_val
                else:
